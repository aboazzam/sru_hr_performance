 ...
 *
 * كل حالة تقول **مَن ينتظره الطلب الآن**، لا ما جرى له آخر مرة: «مراجعة
 * الموارد البشرية» و«تمت المراجعة» كانتا تصفان ما انتهى للتوّ، فلا يعرف
 * القارئ أعليه فعلُ شيء أم ينتظر غيره.
 *
 * هذه صيغة الغائب — ما يراه كل من ليس هو المنتظَر. ومن كان هو المنتظَر يرى
 * `requestStatusSelfLabels` أدناه.
 */
...
/**
 * ما يراه المنتظَر نفسه، حين يكون هو صاحب الخطوة التالية.
 *
 * بقرار صريح: «اختصر إلى بانتظار المراجعة» — الموارد البشرية لا تحتاج أن
 * تُخبَر أن الانتظار عليها هي، فتكفيها «بانتظار المراجعة»؛ ومن سواها يحتاج
 * معرفة على مَن الانتظار، فيرى العبارة كاملةً.
 *
 * حالةٌ ناقصةٌ هنا تعني أن الصيغتين واحدة، وهو الأصل: `hr_reviewed` مثلًا
 * «بانتظار الاعتماد» للجميع، إذ لا اختصار فيها يُفيد.
 */
export const requestStatusSelfLabels: Partial<Record<RequestStatus, string>> = {
  under_hr_review: "بانتظار المراجعة",
};
...
/**
 * The status as THIS reader should see it.
 *
 * "بانتظار المراجعة" for the people the request is waiting on, the full
 * "بانتظار مراجعة الموارد البشرية" for everyone else — asked for directly.
 *
 * The test is NOT "can you act", because VPRA is a ladder: the approver
 * clears `recommend` too, and would have been told the wait is on them when
 * it is on HR. It is "is this step at YOUR standing" — the viewer's own level
 * for the area equals the level the step calls for, exactly. Someone more
 * senior acting is doing it on another's behalf, so they read the full form,
 * which is what was asked for the admin.
 *
 * Falls back to the third-person label whenever no self label exists, so a
 * status with nothing to shorten needs no entry.
 */
export function requestStatusLabelForViewer(
  status: string,
  permissions: RecruitmentPermissions
): string {
  const self = requestStatusSelfLabels[status as RequestStatus];
  if (!self) return requestStatusLabel(status);

  const awaited = requestTransitions.some(
    (rule) =>
      rule.from === status &&
      !rule.statusAdjacent &&
      (permissions[rule.requires.processArea] ?? "none") === rule.requires.minLevel
  );
  return awaited ? self : requestStatusLabel(status);
}
