...
/** Two header buttons, same shape already established for the career-path and
 * job-titles imports: a plain download link for the empty template, and a
 * modal-based import trigger. */
export function ImportVacanciesExcelForm() {
  const t = useTranslations("VacanciesPage");
  const router = useRouter();
  const dialogRef = useRef<HTMLDialogElement>(null);
  const [state, formAction, pending] = useActionState<VacanciesImportResult | null, FormData>(
    importVacanciesExcel,
    null
  );
  const formRef = useRef<HTMLFormElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [handledState, setHandledState] = useState<VacanciesImportResult | null>(null);
  ...
  return (
    <>
      <a href="/templates/sru-vacancies-import-template.xlsx" download className="sru-btn">
            ...
            {state.summary.rowErrors.length > 0 && (
              <details>
                <summary className="text-red-600">
                  {t("importWarnings", { count: state.summary.rowErrors.length })}
                </summary>
