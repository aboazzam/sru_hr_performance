...
/** "Client can add competencies" (CLAUDE.md §3) -- one competency, its classification/definition/impact, and all 4 behavioral levels in one submission, matching the fact that every real competency in the framework already has all 4 filled in. On the shared AddFormDialog (2026-08-29). */
export function AddCompetencyForm({
  ...
  return (
    <AddFormDialog
      dialogRef={dialogRef}
      triggerLabel={t("addCompetencyTriggerButton")}
      heading={t("addCompetencyHeading")}
    ...
    >
      ...
      <div className="sru-form-submitrow">
        <button
          type="button"
          disabled={isPending || !nameAr.trim() || !definitionAr.trim() || !expectedImpactAr.trim() || behavioralLevelOrder.some((l) => !levels[l].trim())}
          onClick={handleAdd}
